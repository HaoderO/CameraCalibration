...
int main( int argc, char** argv )
{
    ...
    bool undistortImage = false; //s是否需要去畸变
    int flags = 0;  //标定flag
    VideoCapture capture; //图片读取
    bool flipVertical; //是否水平翻转
    bool showUndistorted; //是否显示去畸变
    bool videofile;//是否是video文件
    int delay; //摄像头采集延迟，图片读取时无效
    clock_t prevTimestamp = 0;
    int mode = DETECTION; //模式
    int cameraId = 0; //相机ID
    ...
    cv::CommandLineParser parser(argc, argv,
        "{help ||}{w|9|}{h|6|}{pt|chessboard|}{n|10|}{d|1000|}{s|1|}{o|out_camera_data.yml|}"//默认参数，棋盘格宽高，帧数和拍摄延迟，棋盘格尺寸 calibrate.exe -w=
        "{op||}{oe||}{zt||}{a||}{p||}{v||}{V||}{su|1|}"
        "{oo||}{ws|11|}{dt||}"
        "{@input_data|calibration.yml|}");
    if (parser.has("help"))
    {
        help(argv);
        return 0;
    ...
    squareSize = parser.get<float>("s"); //棋盘格尺寸
    nframes = parser.get<int>("n"); //帧数
    delay = parser.get<int>("d"); //拍摄延时
    writePoints = parser.has("op"); //是否输出点
    writeExtrinsics = parser.has("oe");//是否输出外参
    bool writeGrid = parser.has("oo");//是否输出grid
    //如果存在a，那么固定skew
    if (parser.has("a")) {
    ...
    showUndistorted = parser.has("su"); //是否输出去畸变结果，默认为输出
    ...
    //读图和标定主循环，循环次数不超过nframe
    for(i = 0;;i++)
    {
        Mat view, viewGray;
        bool blink = false;

        if( capture.isOpened() )  //如果使用摄像头huozhe video，拍摄图片
        {
            Mat view0;
            capture >> view0;
            view0.copyTo(view);
        }
        else if( i < (int)imageList.size() ) //否则读取图片
       ...
       // improve the found corners' coordinate accuracy
        ...
        {
            imagePoints.push_back(pointbuf); //存储角点
            prevTimestamp = clock();
    ...
    //如果并没有打开摄像头并且需要显示去畸变结果
    if( !capture.isOpened() && showUndistorted )
    {
        Mat view, rview, map1, map2;
        initUndistortRectifyMap(cameraMatrix, distCoeffs, Mat(),
                                getOptimalNewCameraMatrix(cameraMatrix, distCoeffs, imageSize, 1, imageSize, 0),  //保留黑边，获取最大fov去畸变图片
